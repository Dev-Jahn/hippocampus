...
def _seed_dispatch(tmp_project, run_waystone, did="d001"):
    proc = run_waystone(
        [
            "log",
            "dispatch",
...
def _seed_directive(tmp_project, run_waystone, did="gpu-01"):
    proc = run_waystone(
...
def test_bare_task_equals_task_list(tmp_project, run_waystone):
    add = run_waystone(
        ["task", "add", "feat/bare", "--title", "bare default"], cwd=tmp_project
    )
    assert add.returncode == 0, add.stderr
    bare = run_waystone(["task"], cwd=tmp_project)
    full = run_waystone(["task", "list"], cwd=tmp_project)
    assert bare.returncode == 0 and full.returncode == 0
    assert full.stdout.strip() != ""
    assert bare.stdout == full.stdout


def test_bare_log_equals_log_tail(tmp_project, run_waystone):
    _seed_dispatch(tmp_project, run_waystone)
    bare = run_waystone(["log"], cwd=tmp_project)
    full = run_waystone(["log", "tail"], cwd=tmp_project)
    assert bare.returncode == 0 and full.returncode == 0
    assert full.stdout.strip() != ""
    assert bare.stdout == full.stdout


def test_bare_directive_equals_directive_list(tmp_project, run_waystone):
    _seed_directive(tmp_project, run_waystone)
    bare = run_waystone(["directive"], cwd=tmp_project)
    full = run_waystone(["directive", "list"], cwd=tmp_project)
    assert bare.returncode == 0 and full.returncode == 0
    assert full.stdout.strip() != ""
    assert bare.stdout == full.stdout


def test_bare_prior_equals_prior_show(tmp_project, run_waystone):
    (tmp_project / ".waystone" / "PRIORS.md").write_text(
        "# PRIORS\n\n- kernel-impl: sol/high가 실측 우세\n", encoding="utf-8"
    )
    bare = run_waystone(["prior"], cwd=tmp_project)
    full = run_waystone(["prior", "show"], cwd=tmp_project)
    assert bare.returncode == 0 and full.returncode == 0
    assert "PRIORS" in full.stdout
    assert bare.stdout == full.stdout


def test_bare_noun_with_flag_gets_default_sub(tmp_project, run_waystone):
    """A leading '-' token (not help) still triggers insertion:
    `waystone log --ev dispatch` == `waystone log tail --ev dispatch`."""
    _seed_dispatch(tmp_project, run_waystone)
    _seed_directive(tmp_project, run_waystone)
    bare = run_waystone(["log", "--ev", "dispatch"], cwd=tmp_project)
    full = run_waystone(["log", "tail", "--ev", "dispatch"], cwd=tmp_project)
...
def test_directive_add_autoid_roundtrip(tmp_project, run_waystone):
    add = run_waystone(
    ...
    active = run_waystone(["directive", "list", "--active", "--json"], cwd=tmp_project)
    assert active.returncode == 0, active.stderr
    assert did in {d["id"] for d in json.loads(active.stdout)}

    retract = run_waystone(["directive", "retract", did], cwd=tmp_project)
...
def test_directive_add_without_text_and_id_rejected(tmp_project, run_waystone):
    proc = run_waystone(["directive", "add", "--scope", "phase"], cwd=tmp_project)
    assert proc.returncode != 0
    assert proc.stderr.strip() != ""


def test_directive_retract_unknown_id_rejected(tmp_project, run_waystone):
    proc = run_waystone(["directive", "retract", "no-such-id"], cwd=tmp_project)
    assert proc.returncode != 0
    assert proc.stderr.strip() != ""
...
def test_log_tail_ev_filter(tmp_project, run_waystone):
    _seed_dispatch(tmp_project, run_waystone, "d-f1")
    _seed_directive(tmp_project, run_waystone, "f-dir")
    _seed_dispatch(tmp_project, run_waystone, "d-f2")

    proc = run_waystone(["log", "tail", "--ev", "dispatch"], cwd=tmp_project)
    assert proc.returncode == 0, proc.stderr
    ...
    none = run_waystone(["log", "tail", "--ev", "review"], cwd=tmp_project)
    assert none.returncode == 0
    assert none.stdout == ""


def test_log_tail_n_limits_to_last_lines(tmp_project, run_waystone):
    for did in ("d-n1", "d-n2", "d-n3"):
        _seed_dispatch(tmp_project, run_waystone, did)
    proc = run_waystone(["log", "tail", "-n", "1"], cwd=tmp_project)
    assert proc.returncode == 0, proc.stderr
...
def test_removed_surfaces_rejected_inside_project(tmp_project, run_waystone):
    before = read_ledger(tmp_project)
    for args in REMOVED_SURFACES:
        proc = run_waystone(args, cwd=tmp_project)
...
def test_removed_surfaces_silent_outside_project(uninitialized_dir, run_waystone):
    for args in REMOVED_SURFACES:
...
def test_task_dash_h_shows_task_usage(tmp_project, run_waystone):
    proc = run_waystone(["task", "-h"], cwd=tmp_project)
    assert proc.returncode == 0, proc.stderr
    assert "usage: waystone task" in proc.stdout
...
def test_noun_help_lists_subcommands_even_outside_project(
    ...
    for noun, marker in (
        ...
        for flag in ("-h", "--help"):
            proc = run_waystone([noun, flag], cwd=uninitialized_dir)
            assert proc.returncode == 0, f"{noun} {flag}: {proc.stderr}"
            assert f"usage: waystone {noun}" in proc.stdout
            assert marker in proc.stdout, f"{noun} {flag}: {proc.stdout!r}"


def test_top_level_help_states_the_mental_model(uninitialized_dir, run_waystone):
    proc = run_waystone(["--help"], cwd=uninitialized_dir)
    assert proc.returncode == 0
...
def test_prior_distill_regenerates_priors_md(tmp_project, run_waystone, tmp_path):
    _seed_dispatch(tmp_project, run_waystone)
    ...
    proc = run_waystone(
        ...
        cwd=tmp_project,
        env={"WAYSTONE_CLERK_BACKEND": "mock", "WAYSTONE_MOCK_OUTPUT": str(mock)},
    )
    assert proc.returncode == 0, proc.stderr

    priors = tmp_project / ".waystone" / "PRIORS.md"
    ...
    show = run_waystone(["prior", "show"], cwd=tmp_project)
