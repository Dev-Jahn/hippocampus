...
DESIGN.md §3.5 (steps 4-7):
    backend resolution honors WAYSTONE_CLERK_BACKEND=mock (test-only) reading
    WAYSTONE_MOCK_OUTPUT as the clerk's raw stdout.
...
The exit code of `waystone scribe` itself on a *clerk validation* failure is
...
def test_scribe_schema_invalid_output_dumped_and_ledger_gets_only_clerk_failure(
    tmp_project, run_waystone, fake_transcript, schema_invalid_mock_output
):
    """Valid JSON envelope, but an event that fails `waystone log` validation
